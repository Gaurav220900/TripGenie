...
import CafesRecommendations from "./Cafes/CafesRecommendations";
import WeatherWidget from "./Weather/WeatherWidget";
import { jsPDF } from "jspdf";

export default function TripModal({ modalOpen, setModalOpen, currentPlan, chatPlan, destination }) {
  ...
  const downloadPDF = () => {
    ...
    doc.text(`Trip to ${destination || "Your Destination"}`, pageWidth / 2, y, { align: "center" });
    y += 8;

    doc.setFontSize(11);
    doc.setFont("helvetica", "normal");
    doc.setTextColor(100);
    doc.text(`${plan.length}-Day Itinerary  •  Generated by TripGenie`, pageWidth / 2, y, { align: "center" });
    doc.setTextColor(0);
    y += 12;
    ...
    plan.forEach((day) => {
      ...
      day.activities?.forEach((act) => {
        ...
        doc.setFontSize(10);
        doc.setFont("helvetica", "bold");
        doc.text(`${act.time || ""}  ${act.title || ""}`, 18, y);
        y += 5;
        ...
        if (act.desc) {
          ...
          doc.setFontSize(9);
          doc.setTextColor(80);
          const lines = doc.splitTextToSize(act.desc, pageWidth - 36);
          doc.text(lines, 18, y);
          doc.setTextColor(0);
    ...
    doc.save(`TripGenie-${destination || "itinerary"}.pdf`);
  };

  const handleAddCafeToItinerary = (cafe, dayNumber, timeSlot) => {
    const timeMap = { Morning: "9:00 AM", Lunch: "12:30 PM", Evening: "5:00 PM", Night: "7:00 PM" };
    const newActivity = {
      id: `cafe-${cafe.id}-${Date.now()}`,
      title: `☕ ${cafe.name}`,
      time: timeMap[timeSlot] || timeSlot,
      desc: `Visit ${cafe.name}${cafe.address ? ` at ${cafe.address}` : ""}${cafe.rating ? `. Rated ${cafe.rating}/5` : ""}.`,
    };
    const updated = (prev) =>
      prev.map((day) =>
        day.day === dayNumber
    ...
    setPlan((prev) => updated(prev));
    setDisplayedPlan((prev) => updated(prev));
  };
  ...
  // Chat updates — directly update plan without typewriter
  useEffect(() => {
    if (Array.isArray(chatPlan) && chatPlan.length > 0) {
      if (typingRef.current) clearTimeout(typingRef.current);
      setIsTyping(false);
      setPlan(chatPlan);
      setDisplayedPlan(chatPlan);
    }
  }, [chatPlan]);
  ...
  return (
    <>
      {showWeather && (
        <WeatherWidget destination={destination} onClose={() => setShowWeather(false)} />
      )}
      {/* Overlay */}
      {!minimized && (
        <div
        ...
        {!minimized && (
          <div className="p-4 overflow-y-auto flex-1">
                    ...
                    >
                      <div className="flex justify-between items-start mb-2">
                        ...
                        <div className="flex gap-3 items-center">
                          ...
                          {/* Weather Button */}
                          <button onClick={() => setShowWeather(true)} className="bg-blue-100 hover:bg-blue-200 px-3 py-2 rounded-lg transition-colors">
            ...
            {activeTab === 'hotels' && (
                ...
                <PersonalizedForm onSubmit={setPersonalizedFormData} />
            ...
            {activeTab === 'cafes' && (
              <div className="space-y-4">
                <CafesRecommendations
                    itinerary={plan}
                    destination={destination}
                    onAddToItinerary={handleAddCafeToItinerary} />
                </div>
            )}
          </div>
        )}
